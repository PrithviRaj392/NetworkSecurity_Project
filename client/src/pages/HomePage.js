...
const HomePage = () => {
  ...
  const authenticationRequest = async (encryptedChallengeBase64) => {
    try {
      ...
      const encryptedChallengeValue = base64StringToUint8Array(encryptedChallengeBase64);
      const decryptedChallengeValue = await ntru.decrypt(encryptedChallengeValue, ntruPrivateKey);
      const signature = await dilithium.signDetached(decryptedChallengeValue, dilithiumPrivateKey);
      ...
      const response = await axios.post('http://localhost:5000/challenge', { 
        challengeResponseBase64: uint8ArrayToBase64String(decryptedChallengeValue), 
